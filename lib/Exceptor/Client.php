...
class Exceptor_Client
{
    ...
    public function __construct($options_or_dsn=null, $options=array())
    {
        ...
        $this->logger = Exceptor_Util::get($options, 'logger', 'php');
        $this->server = Exceptor_Util::get($options, 'server');
        $this->secret_key = Exceptor_Util::get($options, 'secret_key');
        $this->public_key = Exceptor_Util::get($options, 'public_key');
        $this->project = Exceptor_Util::get($options, 'project', 1);
        $this->auto_log_stacks = (bool) Exceptor_Util::get($options, 'auto_log_stacks', false);
        $this->name = Exceptor_Util::get($options, 'name', Exceptor_Compat::gethostname());
        $this->site = Exceptor_Util::get($options, 'site', $this->_server_variable('SERVER_NAME'));
        $this->tags = Exceptor_Util::get($options, 'tags', array());
        $this->release = Exceptor_Util::get($options, 'release', null);
        $this->environment = Exceptor_Util::get($options, 'environment', null);
        $this->trace = (bool) Exceptor_Util::get($options, 'trace', true);
        $this->timeout = Exceptor_Util::get($options, 'timeout', 2);
        $this->message_limit = Exceptor_Util::get($options, 'message_limit', self::MESSAGE_LIMIT);
        $this->exclude = Exceptor_Util::get($options, 'exclude', array());
        $this->severity_map = null;
        $this->shift_vars = (bool) Exceptor_Util::get($options, 'shift_vars', true);
        $this->http_proxy = Exceptor_Util::get($options, 'http_proxy');
        $this->extra_data = Exceptor_Util::get($options, 'extra', array());
        $this->send_callback = Exceptor_Util::get($options, 'send_callback', null);
        $this->curl_method = Exceptor_Util::get($options, 'curl_method', 'sync');
        $this->curl_path = Exceptor_Util::get($options, 'curl_path', 'curl');
        $this->curl_ipv4 = Exceptor_Util::get($options, 'curl_ipv4', true);
        $this->ca_cert = Exceptor_Util::get($options, 'ca_cert', $this->get_default_ca_cert());
        $this->verify_ssl = Exceptor_Util::get($options, 'verify_ssl', true);
        $this->curl_ssl_version = Exceptor_Util::get($options, 'curl_ssl_version');
        $this->trust_x_forwarded_proto = Exceptor_Util::get($options, 'trust_x_forwarded_proto');

        $this->processors = $this->setProcessorsFromOptions($options);

        $this->_lasterror = null;
        $this->_user = null;
        $this->context = new Exceptor_Context();

        if ($this->curl_method == 'async') {
            $this->_curl_handler = new Exceptor_CurlHandler($this->get_curl_options());
        }
    }
    ...
    {
        return array(
            'Exceptor_SanitizeDataProcessor',
        );
    }

    /**
     * Sets the Exceptor_Processor sub-classes to be used when data is processed before being
     * sent to Sentry.
     *
     * @param $options
     * @return array
    ...
    {
        $processors = array();
        foreach (Exceptor_Util::get($options, 'processors', self::getDefaultProcessors()) as $processor) {
    ...
    /**
     * Parses a Exceptor-compatible DSN and returns an array of its values.
     *
     * @param string    $dsn    Exceptor compatible DSN: http://exceptor.readthedocs.org/en/latest/config/#the-sentry-dsn
    ...
    public function captureException($exception, $culprit_or_options=null, $logger=null, $vars=null)
    {
        ...
        do {
            ...
            if (!class_exists('Exceptor_Stacktrace')) {
                spl_autoload_call('Exceptor_Stacktrace');
            }

            $exc_data['stacktrace'] = array(
                'frames' => Exceptor_Stacktrace::get_stack_info(
    ...
    {
        // manually trigger autoloading, as it's not done in some edge cases due to PHP bugs (see #60149)
        if (!class_exists('Exceptor_Serializer')) {
            spl_autoload_call('Exceptor_Serializer');
        }

        $data = Exceptor_Serializer::serialize($data);
    }

    /**
     * Process data through all defined Exceptor_Processor sub-classes
     *
    ...
    public function sendUnsentErrors()
    {
        ...
        if ($this->store_errors_for_bulk_send) {
            ...
            $this->store_errors_for_bulk_send = !defined('EXCEPTOR_CLIENT_END_REACHED');
        }
    }
    ...
    public function send($data)
    {
        ...
        $message = Exceptor_Compat::json_encode($data);
        ...
        $client_string = 'exceptor-php/' . self::VERSION;
    ...
    protected function get_curl_options()
    {
        $options = array(
            ...
            CURLOPT_USERAGENT => 'exceptor-php/' . self::VERSION,
        );
        if ($this->http_proxy) {
     ...
     * @param string    $client         HTTP client name (not Exceptor_Client object)
    ...
    public function translateSeverity($severity)
    {
        ...
        switch ($severity) {
            case E_ERROR:              return Exceptor_Client::ERROR;
            case E_WARNING:            return Exceptor_Client::WARN;
            case E_PARSE:              return Exceptor_Client::ERROR;
            case E_NOTICE:             return Exceptor_Client::INFO;
            case E_CORE_ERROR:         return Exceptor_Client::ERROR;
            case E_CORE_WARNING:       return Exceptor_Client::WARN;
            case E_COMPILE_ERROR:      return Exceptor_Client::ERROR;
            case E_COMPILE_WARNING:    return Exceptor_Client::WARN;
            case E_USER_ERROR:         return Exceptor_Client::ERROR;
            case E_USER_WARNING:       return Exceptor_Client::WARN;
            case E_USER_NOTICE:        return Exceptor_Client::INFO;
            case E_STRICT:             return Exceptor_Client::INFO;
            case E_RECOVERABLE_ERROR:  return Exceptor_Client::ERROR;
        }
        if (version_compare(PHP_VERSION, '5.3.0', '>=')) {
            switch ($severity) {
            case E_DEPRECATED:         return Exceptor_Client::WARN;
            case E_USER_DEPRECATED:    return Exceptor_Client::WARN;
          }
        }
        return Exceptor_Client::ERROR;
    }
